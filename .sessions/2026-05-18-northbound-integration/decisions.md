...
## D008: TestReport 内容改为配置驱动(推翻 H012 的"从执行链路采集"方向)

> status: active
> date: 2026-06-28
> 取代:H012 的 reportDataCollector 接线方向(handoff 计划但未实施)
> 被取代:无

### 决策

TestReport(甲方第三层报告)的 checkPoints / statisticsItems / attachItems 三类内容,由**每用例一份独立的"报告配置清单"驱动**:用户在用例目录(CatalogMappingPanel 的映射展开区)配置"报哪些接收帧字段的值",task 跑完时按这份清单去 **DisplayService 的字段值快照**取 displayValue(解析值,如"锁定"非原始 `0x01`),填进甲方的三类。配置数据归 command-ingress feature,northbound 跨读。

**五个核心设计点(用户 brainstorm 拍板)**:
...
H012 handoff 计划"接 reportDataCollector 从 task 执行链路采集真实数据",经 brainstorm 验证为**错误方向**:

- **抽象层错位**:执行链路采的是"帧字段匹配结果"(step-N / 0x01),甲方 checkPoints 要的是带业务语义的指标(载波同步锁定/误码率),采集出来的字段没业务意义。
- **缺配置维度**:报告该报哪些字段是用户决定的,执行链路自动推断不出来。
- **reportDataCollector 废弃**:本设计不接 reportDataCollector(feature-wiring 不传),那套从 step 事件采 wait/send 的逻辑不再需要。report-data-collector.ts / spec 暂不删(避免范围爆炸),标记 dead code 后续清理。
- **DEFAULT_MOCK_CONFIG 假数据**:此前甲方收到的 checkPoints("上电状态/载波同步锁定/帧同步锁定/误码率"、testValue 永远 0x0001/0.2%)就是这份写死常量(test-report-generator.ts:61-95)。本设计改为配置驱动;没配 ReportConfig 的用例三类为空,**不 fallback 假数据**(诚实空着)。

### 排除的替代方案

- ❌ **reportDataCollector 从执行链路采集**(H012 方向):抽象层错位 + 缺配置维度,如上。否决。
- ❌ **checkPoint 自动从 task 的 wait-condition step 派生**:用户明确"字段不一定 wait,报告和 wait 完全无关",数据源不能绑定执行链路。否决。
- ❌ **报告配置挂 CatalogMapping**(耦合):用户"所有用例都能配报告,别搞耦合"——配置要独立于 enabled/overridablePaths,不塞进映射对象。否决。
- ❌ **新建独立 report-config feature**:为一个配置清单划地盘过度,同 D004 架构(command-ingress 持配置 + northbound 跨读)即可。否决。
- ❌ **取原始值(0x01)非 displayValue**:用户明确"表格里显示啥就是啥"(锁定非 0x01),复用 display 解析。否决。
- ❌ **做期望值/判定配置**:用户"纯取值",checkPoint 的 expectValue/result 不配。否决(留另一特性)。
...
- 设计 spec:`rewrite/docs/superpowers/specs/2026-06-28-report-config-design.md`
- 实施 handoff(S018 待写):按 spec → writing-plans → 实施

### 来源

- 用户 brainstorm(2026-06-28):报告该报什么、配置形态、取值来源、UI 入口(多轮对话)
- 触发原话: 见 voice.md 2026-06-28
