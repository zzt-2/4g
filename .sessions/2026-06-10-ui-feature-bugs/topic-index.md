...
> 状态: active | 创建: 2026-06-10 | 最后更新: 2026-06-24 S014 任务模板等待条件全覆盖测试数据 + 实测验证:造 3 个 receive 测试帧 + 21 任务模板(覆盖 9 operator + and/or + 3 onTimeout + 永久等待 + 中断 + sourceId + repeat.until + exitCondition),31 集测全过,JSON 经应用真实导入路径验证可导入。发现 1 真 bug(fieldValueProvider 生产未接线→repeat.until/exitCondition 失效)+ 2 行为澄清(onTimeout=fail+stop 终态 stopped 非 failed / 中断 step 不记 result)。前序 S013 编辑实例弹窗深度重设计、S012 storage 性能治本。
...
**S014 任务模板等待条件全覆盖测试数据 + 实测验证(2026-06-24)**。用户原话"帮我测一下任务的那些条件…任务模板的等待条件…需要一套帧定义和任务定义去测、去试它能不能用"。造 `public/test-data/wait-condition-test-{frames,tasks}.json`(3 receive 帧 + 21 任务模板,9 operator 全覆盖 + and/or + 3 onTimeout + 永久等待 + 中断 + sourceId + repeat.until + exitCondition),两文件经应用真实导入路径验证可干净导入。实测 `task/__tests__/wait-condition-coverage.spec.ts` 31 用例跑真实执行路径(FakeReceiveEventSource.emit 模拟收帧)**31/31 全过**。**发现 1 真 bug**:`runtime/feature-wiring.ts:146-152` createTaskService 未传 fieldValueProvider → repeat.until/exitCondition 在生产永不生效(wait-condition step 不受影响);**2 行为澄清**:onTimeout=fail+stop 终态 stopped 非 failed / 中断 step 不记 result(均设计如此)。lint 0 / task 全套 311/311。无 D###。详见 S014。
