...
**S012 接收帧高频卡顿——storage appendLocalRecords O(N·M) 治本(2026-06-23)**。用户原话"一秒接收十几帧…严重的性能问题"+ 补"开久了卡"。主对话 analyze-perf.py 锁 timer#4=routingTick setInterval(300-430ms/次,占主线程几乎全部)。**Phase 1 证据反转**:初读 trace 误判"76次=100ms定时器=一秒十几帧",用户纠正帧速率≠定时器频率;Node 基准证伪主对话嫌疑#2"routingTick 回调重活"(12帧仅5.5ms,服务层纯 JS 无 Vue reactive)。持续推帧采样锁定真凶:**storage `appendLocalRecords` 每帧全量深拷贝+排序=O(N·M)**,2000 records 单 tick 62ms(56x),反推 trace 300ms 自洽(15分钟~万 records)。**治本**(D010):`canAppendInOrderFast` 快速路径(单调递增+id 全新时 append-only)+ `storage-state` 加 `appendRecords`/`getLastRecordCapturedAtMs`/`hasRecordId` 轻量 accessor + 复用 snapshot。TDD RED(旧34ms fail)→GREEN(新18.65ms),语义不变量全过。端到端 62ms→34ms(1.8x)。顺带清 9 处 debug log + 修 pre-existing 测试基建(vitest 加 vue 插件,receive/runtime/storage 89 测试从 broken 恢复)。tsc 0 / lint 0 / storage+receive+command-ingress 181 / runtime+append-perf 17。pre-existing 5 集成失败(event-truncation+tcp-datapath)HEAD 确认无关。**待用户目标机实测**:接收十几帧/秒跑 15+ 分钟看卡顿缓解。详见 S012 + D010。
