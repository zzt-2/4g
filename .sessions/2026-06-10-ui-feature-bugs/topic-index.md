...
> 状态: active | 创建: 2026-06-10 | 最后更新: 2026-06-25 **S012 续接二轮:接收帧高频卡顿真根因彻底治本**。新证据"拔串口立刻好"定性钉死卡顿 100% 来自帧流入后处理路径。analyze-longtask.py 扒 617ms Long Task 内部=纯 timer#4 JS(consoleCall=0 推翻 log 假设)。prod-scale 基准暴露 D010-B 只治 merge+sort、漏 3 次全量深拷贝(含隐藏元凶 setLastIssue 返 snapshot):10k=99ms/20k=203ms 完美 O(N)。治本(D010 续):分离 appendRoutedRecords(snapshot-free)+mutable push+id Set+clearLastIssue+攒批写盘,路由路径 20k:49.8ms→0.01ms(O(1),降 5000x)。163/163 测试过 + tsc0 + lint0。**待用户目标机实测**。前序 S014 深查 echo 回环发两次才更新 bug、S014 续接三处修复(D012)、S013、S012。
