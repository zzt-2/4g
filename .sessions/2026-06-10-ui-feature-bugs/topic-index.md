...
> 状态: active | 创建: 2026-06-10 | 最后更新: 2026-06-26 **S015:routingTick 僵尸写入路径根除(推翻 D010 续接二轮)**。用户 prod 运行埋点实锤 `storage×6(peak 894ms)` 随 N 线性涨——D010 续接二轮的"攒批治本"没解决(攒批照样 O(N),还把"每帧小写"变"每50条一次大写",峰值更高)。决定性新症状"连上立刻卡/断开立刻好/必须重启"+ 用户直觉"没开记录不该写"触发重新审视:代码三方实锤 B 路径(routingTick→fanOutToStorage→appendRoutedRecords)是**僵尸写入**——key 是裸 fieldName 与 History 页 4 段格式不符(永远画不出)、无开关(不管开没开记录都写)、生产用 fake adapter 从不落盘(卡顿是内存 N 无限增长+攒批全量深拷贝)。**砍掉 B 路径全部死代码 + startTickDriver 加 in-flight guard 防并发雪崩**。records 唯一合法入口是 DisplayPage 录制按钮(路径A,appendLocalRecords,保留)。D010 标记 partially-superseded(续接二轮部分推翻,内存优化保留给路径A)。8文件39过,全量11failed全baseline(stash比11=11),tsc0,lint0。**待用户目标机实测**:连串口看 storage 段警告消失。前序 S012 续接二轮(被推翻)、S014 续接(D012)、S013、S012。
...
- **S014** 任务模板等待条件全覆盖测试数据 + 实测验证 (06-24,子对话造数据+实测)：用户原话"帮我测一下任务的那些条件…任务模板的等待条件…需要一套帧定义和任务定义去测、去试它能不能用"。**造测试数据**:`public/test-data/wait-condition-test-frames.json`(3 receive 帧 wt-status/wt-aux/wt-counter,覆盖数值 uint16/十六进制 uint32/字符串枚举/可空/多 frameId 隔离)+ `wait-condition-test-tasks.json`(21 任务模板,operator 覆盖全 9 种 any/change/contains/eq/gt/gte/lt/lte/neq + and/or + 3 onTimeout + 永久等待 + 中断 + sourceId + repeat.until + exitCondition)。两文件均经应用真实导入路径(`deserializeFrames`+`validateFrameAssetCollection` / `parseImportedFile`)验证 `ok/issues:[]` 可干净导入。**实测**:`task/__tests__/wait-condition-coverage.spec.ts` 31 用例跑真实执行路径(createTaskService→executeWaitConditionStep→ConditionRegistry.processInput,FakeReceiveEventSource.emit 模拟收帧),**31/31 全过**。**发现 1 真 bug + 2 行为澄清**(均报告不擅自修):①【真 bug】`runtime/feature-wiring.ts:146-152` createTaskService 未传 fieldValueProvider → repeat.until(task-iteration-loops.ts:222 被跳过)和 exitCondition(:125 return false)在生产永不生效,wait-condition step 不受影响(走 push registry);②【非 bug,设计如此但反直觉】onTimeout=fail+errorPolicy:stop 终态是 stopped(task-error-policy.ts:32)非 failed,只有 retry 用尽才 failed;③【非 bug,设计如此】被中断的 wait/delay step 不记 step result(task-iteration-loops.ts:338 interrupted→null)。push 模型不看历史帧是设计如此(group 在 task start 时就建,影响小)。sourceId UI 缺失按边界不当 bug 报但 JSON 可配且生效。lint 0 / task 全套 311/311。无 D###(测试/验证+bug 报告,无架构决策)。详见 S014。**待用户**:fieldValueProvider 缺口是否单独开任务修。
- **S014 续接:三处修复落地** (06-24,用户拍板"全部一起修"):见 **D012**。① fieldValueProvider 生产接线(真 bug):`feature-wiring.ts:146` 加 `fieldValueProvider: () => bridge.getLatestFieldValues()` + `receive-event-source-bridge.ts` 加 `getLatestFieldValues()`(emit 时按 fieldId 扁平 merge)→ repeat.until/exitCondition 生产生效。② onTimeout=fail 终态改 failed(符合直觉):`task-error-policy.ts:32` 'stop' 分支 `updateLifecycle('stop')`→`'fail'`,手动 stopTask 不变(仍 stopped)。③ KPI 加「已停止」计数:`ExecutionKpiBar.vue` 补 stopped 项。④ 数据修正:frames.json 补 2 send 镜像帧(wt-status-cmd/wt-counter-cmd,字节布局对齐 receive 帧供 echo 回环)+ tasks.json 修 repeat.until/exitCondition 的 send step 引用(原误引 receive 帧)+ 新增 echo server 手测 README(应用无内置回环入口)。验证:task **317/317** + 受影响集成(task-error-strategies/command-ingress-task-ack-chain/connection-lifecycle-boundaries)**19/19** + wait-condition-coverage **37/37**(新增 6 回归:bridge.getLatestFieldValues + repeat.until/exitCondition 接线生效 + 手动 stopTask 仍 stopped)。终态断言更新 10 处(errorPolicy stop→failed,手动 stop 不变)。JSON 5 帧(2 send+3 receive)ok:issues:0 / 21 模板可解析 / 所有 send step 引 send 帧。lint 0 / tsc 0。**并发**:HEAD 已被并行会话推进 8 commit(northbound docking/command-ingress),无冲突。pre-existing 5 失败(tcp-receive-datapath 4+event-truncation 1)baseline 比对确认无关。详见 S014 续接 + D012
- **H011** 接收帧卡顿性能优化——交接(三症状三修复均未解决,根因待新 trace 锁定) (06-24)：用户原话"还是不行。依然卡。我打算给你新的文件让你看看。此外,目前上下文爆了,你先写个交接文档"。S012 的三个修复(storage append-only O(N·M) 治本 `2ec0ca5` / 清 9 处 debug log `2ec0ca5` / backgroundThrottling:false `c882a04`)**全部被证伪——prod 仍卡**。**核心失败教训**:所有定位证据来自"代码 + Node 基准",复现不出 prod 的 300ms(缺真 Electron IPC 适配器 / detached devtools / 硬件帧流 / Vue 渲染层)。**新对话别再走已排除方向**(routingTick 服务层/backgroundThrottling/storage 是唯一主因)。**等用户给新文件**(可能带 source map 的 dev trace)→ 优先看 Long Task 内部热点函数落点 → 重点查之前没查的 Vue 渲染层(DisplayPage 表格/统计/图表 reactive 重算)+ 考虑用户是否开着 devtools 自身开销。若需测渲染层,项目当前**无 happy-dom/jsdom/@vue-test-utils**,需先搭。诚实优先:定位不准就直说,别再靠代码推断。详见 H011-receive-perf-handoff.md(含失败数据附录+接口变更+已知债务+验证阈值+硬性建议)。
- **S012 续接二轮:接收帧高频卡顿真根因彻底治本** (06-25)：H011 后续。用户给新 trace(25MB)+ 决定性证据"**拔串口立刻好**" → 定性钉死卡顿 100% 来自帧流入后处理路径(用户直觉"某些东西触发太多"被证实)。写 `analyze-longtask.py` 扒 617ms Long Task 内部:**616.7ms 全是单个 timer#4 回调纯 JS,consoleCall=0**(推翻 H011"每帧 console.warn"假设;GC 仅 40ms;0% Layout/Paint)。**prod-scale 基准暴露 H011/D010-B 致命盲区**:前三轮 Node 基准只测 state.appendRecords 的 merge+sort(5.5ms),**漏了 service 完整调用链的 3+1 次全量深拷贝**(getSnapshot 备份 / appendRecords 返 snapshot / adapter cloneStorageValue / **隐藏元凶 setLastIssue(undefined) 内部 return snapshot()**)。warmup 只 2000 records 看不到真实 O(N)。prod-scale(10k/20k)实测:10k=99ms / 20k=203ms 完美 O(N) 线性,反推 prod 600ms≈60k records。**治本(D010 续,用户拍板 A 全做)**:① 分离 `appendRoutedRecords`(snapshot-free,只返 `{ok}`;routingTick 经 fanOutToStorage 调它),`appendLocalRecords` 保完整契约(立即写+失败感知)给其它调用方;② `appendRecords` 改 mutable push(O(新条数))取代 `[...全量,...新]`(O(N) 重建);③ `hasRecordId` 改 Set 索引(O(1))取代 O(N) 遍历;④ 新增 `clearLastIssue`(O(1) 不返 snapshot)取代路由路径 `setLastIssue(undefined)`(返 snapshot=全量深拷贝,隐藏元凶);⑤ 写盘失败回滚用 `truncateRecords`(`length=count`,O(1));⑥ 攒批写盘(appendRoutedRecords 累 dirty 计数,阈值 50 才全量 writeMaterial,摊销 O(1)/帧;runtime.destroy 调 flushPendingWrites 防丢数据)。**效果**:路由路径 5k=0.01ms / 20k=0.01ms(旧 11.5/49.8ms),ratio 0.58(O(1)),**降 5000 倍**。prod 60k 单帧 ~600ms→~0.01ms。TDD RED(旧 20k=193ms FAIL)→GREEN。**163/163 测试过** + tsc0 + lint0(5 源+5 测试 mock 改:fanout-consumer-order/routing-tick-regression/routing-tick-error-isolation/history-fieldname-resolution/runtime/helpers 的 storage mock 从 spy appendLocalRecords 改 spy appendRoutedRecords + 加 flushPendingWrites)。子 agent 扫描确认 receive state 有 bounded() 滚动窗口(O(cap) OK)、display buffer 由 preferences 定义不随时间增长(OK),**storage 是唯一随运行时间无限累积的集合**。**待用户目标机实测**(完全重启 dev server,main 改动 HMR 不生效)。详见 S012 续接二轮 + D010 续。**⚠️ 2026-06-26 续接:S012 续接二轮被 S015/D013 推翻**——攒批治本是假象(测的不触发攒批那帧),prod 实测仍卡;且治的是本不该存在的僵尸写入路径。详见 S015。
- **S015 routingTick 僵尸写入路径根除 + tick 防雪崩(推翻 D010 续接二轮)** (06-26)：S012 续接二轮后续。用户在 prod 跑运行埋点(routingTick 四段计时)实锤 `[routingTick] slow over 5s: storage×6(peak 894ms) | peakEvents=13`——**storage 段占 99%,随 peakEvents 线性涨(13事件~1s、27事件~1.5s)**,drain/parse/display 全个位数 ms 是噪声。新决定性症状"**连串口立刻卡/断开立刻好/必须重启**"排除"内存随会话增长"(连上第一个 tick 就卡),钉死写入触发。用户直觉"**我没开记录才对,这有什么要记的吗**"触发对 B 路径(routingTick→fanOutToStorage→appendRoutedRecords)存在必要性的质疑。**代码三方实锤 B 路径是僵尸写入**:① key 是裸 `f.fieldName`(receive-storage-bridge.ts:14),History 页 `useHistoryData.ts:122` 要 4 段 `groupId:frameId:fieldId:fieldName` 才画点——B 数据 History 永远画不出;② 无开关(不管开没开记录都写,DisplayPage 录制按钮是另一条路径A);③ 生产用 fake adapter(feature-wiring.ts:101,real adapter 零引用,bootstrap 不调 loadLocalRecords),records **从不落盘**——卡顿是内存 N 无限增长 + 攒批满 50 触发 `cloneStorageValue` 全量深拷贝 = O(N);④ `setInterval(100ms)` 无并发保护,单 tick 超 100ms 后并发 tick 各自抢全量深拷贝 → 雪崩冻屏 → 必须重启。**D010 续接二轮为何没治本且该推翻**:声称"降5000倍"是测不触发攒批那帧的 0.01ms 假象;攒批没治本反而把"每帧小写"变"每50条一次大写"峰值更高;否决"滚动上限"的前提错(以为 B 数据给 History 用,实际是僵尸)。**治本(D013,用户拍板"彻底清理+优化路径A")**:① 砍掉 routing-tick.ts 的 fanOutToStorage 调用 + 删 receive-storage-bridge.ts 整文件;② 删 appendRoutedRecords/flushPendingWrites(接口+实现)+ 简化 applyRoutedAppend→appendAndPersist(去 batchWrites 参数+攒批逻辑,恒立即写盘);③ runtime/index.ts destroy 删 flushPendingWrites 调用;④ **startTickDriver 加 in-flight guard**(tickInFlight flag,上一 tick 未完成则跳过本次 setInterval 触发)止血并发雪崩。**保留** appendLocalRecords(路径A,DisplayPage 录制按钮,History 靠它)+ D010 的内存优化(mutable push/Set/clearLastIssue/truncate/canAppendInOrderFast)归入 appendAndPersist。**测试**:删 storage-append-no-growth.spec(测已删方法);4 integration 改删 storage 相关(fanout-consumer-order 去 storage 测试+callOrder 去 fanout-storage / routing-tick-regression 删 BF-1 块 / routing-tick-error-isolation 删 storage failure 测试弱化 display failure);helpers/history mock 清理;routing-tick.spec 去 storageMs 断言。**新增 2 回归**:routing-tick.spec "routingTick 不写 storage(D013)"+ bootstrap-integration "tick 不并发(in-flight guard, 慢 drain mock 验峰值≤1)"。**验证**:8 文件 39/39 过;全量 11failed/1868passed,**stash 基线比对 11=11(0 新增)**;tsc src/ 0 错;lint 我的文件 0 错。11 failed 全 pre-existing(event-truncation 1+tcp-receive-datapath 4+heartbeat-timer 5+frame-service-state-selector 1)。D010 标记 partially-superseded(续接二轮部分推翻,内存优化保留)。**待用户目标机实测**:连串口跑,看 `[routingTick] slow` 的 storage 段是否消失(应只剩 drain/parse/display)。详见 S015 + D013。
...
- **D012**(decisions.md):等待条件三处修复 —— fieldValueProvider 生产接线(bridge.getLatestFieldValues,repeat.until/exitCondition 生效)+ onTimeout=fail 终态改 failed(errorPolicy stop 分支,手动 stop 不变)+ KPI 加已停止计数 + 数据补 send 镜像帧。S014 测试发现 → 用户拍板"全部一起修"。
