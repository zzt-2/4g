...
用户亲自改了两个文件(ExecutionListPage + TaskExecutionDetail),diff 后发现**真正的布局修复只有两点**,其余 95% 是 prettier 格式化(多行属性压单行)。前 4 次续接我堆了大量 `flex-1 min-h-0 overflow-hidden`,其中多数是不必要的。

### 用户真正的两点修复

1. **`TaskExecutionDetail.vue`**:步骤状态区 + 内部滚动列表两个 flex 容器加 **`no-wrap`**(`flex flex-col flex-1 min-h-0 gap-1 no-wrap` 和 `flex flex-col gap-1 min-h-0 overflow-y-auto no-wrap`)。显式 no-wrap 保证子项不换行,让 min-h-0 + overflow-y-auto 正确触发收缩滚动。**根 flex-1 min-h-0 用户保留了我加的**(那层是对的)。
