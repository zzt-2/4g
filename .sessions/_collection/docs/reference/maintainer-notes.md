...
新工作流定型后，在根技能第三节"目前包含的子工作流"里加一段索引，并登记新的目录位置。

### 跨阶段新约束

如果发现某条规则适用于所有阶段（例如所有 spec doc 都必须补某个字段），优先写进共享 reference 或根技能的共同约束，不要只改一个子技能。

### 新模板 / 新产物类型

如果引入新的 spec 产物（例如风险评估表、回滚预案），先在根技能第二节登记路径，再在对应阶段技能里引用。
...
目前查看"项目当前有几个 feature 在进行中、几个 issue 未关闭"仍需要手动查询。未来如要补 `status.py` 或 `easysdd/STATUS.md`，先在根技能登记方向，再实现。

---

## 3. 维护规则

- 每次扩展都要同步更新中心索引和相关子技能
- 不允许只在某个子技能里加东西而不在中心登记
- 共享说明优先放 `easysdd/reference/`，根技能只保留路由和高层约束