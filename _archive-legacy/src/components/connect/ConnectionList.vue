...
<template>
  <div class="flex flex-col no-wrap h-full max-w-[25vw] bg-industrial-panel">
    <!-- 网络连接部分 -->
    <div class="mb-4">
      <div class="flex justify-between items-center p-3 border-b border-industrial bg-industrial-secondary">
        ...
        <q-btn flat round size="sm" icon="refresh"
    ...
    <div class="flex-1 flex flex-col max-h-[70vh]">
      <div class="flex justify-between items-center p-3 border-b border-industrial bg-industrial-secondary">
        ...
        <q-btn flat round size="sm" icon="refresh"
          class="text-industrial-secondary hover:text-industrial-primary hover:bg-industrial-highlight"
          :loading="serialStore.isLoading" @click="serialStore.refreshPorts(true)">
    ...
    <!-- 全局刷新按钮 -->
    <div class="mt-3 p-5 border-t border-industrial bg-industrial-panel">
      <q-btn label="刷新所有连接" class="w-full btn-industrial-primary" outline :loading="serialStore.isLoading"
