<template>
  <div class="bg-industrial-panel border-r border-industrial h-full flex flex-col">
    <!-- 标题 -->
    <div
      class="flex no-wrap justify-between items-center bg-industrial-secondary border-b border-industrial p-3 gap-4"
    >
      <q-input
        v-model="searchText"
        placeholder="搜索"
      ...
      <!-- 导入导出按钮 -->
      <ImportExportActions
        :getData="handleGetScoeData"
        :setData="handleSetScoeData"
        storageDir="data/scoe/satelliteConfigs"
        exportTitle="导出卫星配置"
        importTitle="导入卫星配置"
      />
    </div>
    ...
    <div class="flex-1 min-h-0 p-3 space-y-2 overflow-y-auto">
      <div
        v-for="config in filteredSatelliteConfigs"
        :key="config.id"
        ...
        :class="{
          'bg-industrial-highlight border-industrial-accent':
            scoeStore.selectedConfigId === config.id,
        }"
      ...
      >
        ...
        <div class="space-y-1">
          <div class="flex items-center gap-2">
            ...
            <span
              v-if="satelliteConfigValidationErrors.find((error) => error.id === config.id)?.error"
              class="text-red-5 text-xs"
              >配置不完整</span
            >
          </div>
        ...
        <div class="flex gap-2">
          <q-btn
            ...
            @click.stop="handleCopy(config.id)"
          >
      ...
      <!-- 空状态 -->
      <div
        v-if="filteredSatelliteConfigs.length === 0"
        class="text-industrial-tertiary text-center py-8 text-sm"
      >
        暂无配置项
        <br />
        点击下方按钮添加
      ...
      error:
        sendConfigFields
          .map(
            (field) =>
              field.must &&
              !(config.sendConfig as ScoeSendConfig)[field.key as keyof ScoeSendConfig],
          )
          .includes(true) ||
        receiveConfigFields
          .map(
            (field) =>
              field.must &&
              !(config.receiveConfig as ScoeReceiveConfig)[field.key as keyof ScoeReceiveConfig],
          )
          .includes(true),
    };
  });
