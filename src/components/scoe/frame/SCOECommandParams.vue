  ...
  <div v-if="localCommand && shouldShow" class="flex-1 flex flex-col">
    ...
    <div class="bg-industrial-secondary border border-industrial rounded p-2">
      ...
      <div v-if="localCommand.params?.length" class="space-y-2">
        <div
          v-for="param in localCommand.params"
          :key="param.id"
        ...
        >
          ...
          <div class="flex items-center gap-2 flex-wrap">
            ...
            <!-- 目标帧ID -->
            <q-select
              :model-value="param.targetInstanceId"
              :options="frameInstanceOptions"
              dense
              outlined
              dark
              option-value="id"
            ...
            <!-- 字段ID -->
            <q-select
              :model-value="param.targetFieldId"
              :options="getFieldOptions(param.targetInstanceId)"
              dense
              outlined
              dark
              option-value="id"
            ...
            <q-btn
              ...
              icon="edit"
              color="blue-5"
              @click="store.toggleParamExpansion(param.id)"
            >
              <q-tooltip>{{
          ...
          <!-- 展开的选项列表 -->
          <div
            v-if="store.expandedParamIds.has(param.id)"
          ...
          >
            ...
            <div v-if="param.options?.length" class="space-y-1">
              ...
              >
                ...
                <!-- 接收码 -->
                <q-input
                  :model-value="option.receiveCode"
                  dense
                  outlined
                  dark
                  bg-color="industrial-panel"
    ...
    <div class="bg-industrial-secondary border border-industrial rounded p-2">
      <div class="w-full flex items-center justify-between mb-2">
        ...
        <div class="flex-1 max-w-120 flex no-wrap items-center justify-end gap-4">
          <template v-if="localCommand.function === ScoeCommandFunction.READ_FILE_AND_SEND">
            ...
            <q-input
              v-model="localCommand.sendInterval"
              dense
              outlined
              dark
              bg-color="industrial-panel"
              class="flex-1 min-w-16 text-xs"
              hide-bottom-space
              @update:model-value="
          ...
          <q-label class="flex-1 min-w-0 truncate text-sm text-industrial-secondary"
            >选择帧:
          </q-label>
          <q-select
            v-model="selectedFrameId"
            :options="availableSendFrames"
            dense
            outlined
            dark
            option-value="id"
            ...
            map-options
            bg-color="industrial-panel"
            class="flex-1 flex-grow-3 text-xs"
            hide-bottom-space
          />
          <q-btn
            flat
      ...
      <div v-if="localCommand.frameInstances?.length" class="space-y-2">
        <div
          v-for="(instance, index) in localCommand.frameInstances"
          :key="index"
            ...
            <div class="flex items-center gap-1">
              <div class="flex items-center gap-3 pr-6">
                ...
                <q-select
                  ...
                  map-options
                  bg-color="industrial-panel"
                  class="q-input-class w-48 text-xs"
                  placeholder="选择目标"
                  hide-bottom-space
                >
