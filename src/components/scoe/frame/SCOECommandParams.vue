<template>
  <template v-if="localCommand && shouldShow">
    <!-- 参数配置 -->
      ...
      <div v-if="localCommand.params?.length" class="space-y-2">
        <div v-for="param in localCommand.params" :key="param.id"
          ...
          <div class="flex items-center gap-2 flex-wrap">
            ...
            <q-select :model-value="param.type" :options="paramTypeOptions" dense outlined dark option-value="value"
              ...
              hide-bottom-space @update:model-value="
                store.updateParam(param.id, { type: $event as 'string' | 'number' | 'boolean' })
                " />
            ...
            <q-select :model-value="param.targetInstanceId" :options="frameInstanceOptions" dense outlined dark
              ...
              class="w-48 text-xs" label="目标帧" hide-bottom-space @update:model-value="
                store.updateParam(param.id, { targetInstanceId: $event || undefined })
                " />
            ...
            <q-select :model-value="param.targetFieldId" :options="getFieldOptions(param.targetInstanceId)" dense
              ...
              class="w-48 text-xs" label="目标字段" hide-bottom-space @update:model-value="
                store.updateParam(param.id, { targetFieldId: $event || undefined })
                " />

            <!-- 编辑选项 -->
            <q-btn flat dense round size="xs" icon="edit" color="blue-5" @click="store.toggleParamExpansion(param.id)">
              <q-tooltip>{{
                store.expandedParamIds.has(param.id) ? '收起选项' : '编辑选项'
                }}</q-tooltip>
            </q-btn>
          ...
          <div v-if="store.expandedParamIds.has(param.id)" class="mt-2 p-2 bg-industrial-panel rounded">
            ...
            <div v-if="param.options?.length" class="space-y-1">
              <div v-for="(option, optIndex) in param.options" :key="optIndex"
                ...
                <q-input :model-value="option.label" dense outlined dark bg-color="industrial-panel"
                  class="q-input-class flex-1 text-xs" placeholder="标签" hide-bottom-space @update:model-value="
                    store.updateParamOption(param.id, optIndex, { label: String($event || '') })
                    " />

                <!-- 值 -->
                <q-input :model-value="option.value" dense outlined dark bg-color="industrial-panel"
                  class="q-input-class flex-1 text-xs" placeholder="值" hide-bottom-space @update:model-value="
                    store.updateParamOption(param.id, optIndex, { value: String($event || '') })
                    "><template #prepend>
                    <span class="text-sm">对应值</span>
                  </template></q-input>

                <!-- 接收码 -->
                <q-input :model-value="option.receiveCode" dense outlined dark bg-color="industrial-panel"
                  class="q-input-class flex-1 text-xs" placeholder="接收码(hex)" hide-bottom-space @update:model-value="
                    store.updateParamOption(param.id, optIndex, {
                      receiveCode: String($event || ''),
                    })
                    "><template #prepend>
                    <span class="text-sm">接收码</span>
                  </template></q-input>
            ...
            <div v-else class="text-industrial-tertiary text-xs text-center py-1">暂无选项</div>
          </div>
        </div>
      </div>
      <div v-else class="text-industrial-tertiary text-xs text-center py-2">暂无参数配置</div>
    </div>
      ...
      <div class="w-full flex items-center justify-between mb-2">
        ...
        <div class="flex-1 max-w-120 flex no-wrap items-center justify-end gap-4">
          ...
          <q-input v-model="localCommand.sendInterval" dense outlined dark bg-color="industrial-panel"
            class="flex-1 min-w-24 text-xs" hide-bottom-space @update:model-value="
              store.updateReceiveCommandField('sendInterval', Number($event) || undefined)
              ">
            <template #append>
              <span class="text-xs text-industrial-secondary">ms</span>
            </template>
          </q-input>
          <q-label class="flex-1 min-w-0 truncate text-sm text-industrial-secondary">选择帧:
          </q-label>
        ...
        <div v-for="(instance, index) in localCommand.frameInstances" :key="index"
          ...
          <div class="h-32px flex items-center justify-between rounded">
            <div class="flex items-center gap-2">
              ...
              <span class="text-xs text-industrial-primary">{{
                instance.label || `实例 ${index + 1}`
                }}</span>
            </div>
            <div class="flex items-center gap-1">
              ...
              <q-btn flat dense round size="xs" icon="edit" color="blue-5"
                ...
                <q-tooltip>{{
                  store.expandedInstanceIds.has(String(index)) ? '收起' : '编辑'
                  }}</q-tooltip>
              </q-btn>
              <!-- 复制 -->
          ...
          <div v-if="store.expandedInstanceIds.has(String(index))" class="pt-2">
            <ConfigurableFieldsList :fields="instance.fields" @update:field-value="
              (fieldId, value) => store.updateFrameInstanceField(index, fieldId, value)
            " />
          </div>
        </div>
      </div>
      <div v-else class="text-industrial-tertiary text-xs text-center py-2">暂无帧实例</div>
    </div>
  </template>
</template>
