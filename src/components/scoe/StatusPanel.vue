...
const statusItems: StatusItem[] = [
  ...
  if (item.key === 'healthStatus') {
    return {
      ...item,
      get icon() {
        return statusMaps.health[status.value.healthStatus]?.icon || 'help';
      },
      get iconColor() {
        return statusMaps.health[status.value.healthStatus]?.color || 'text-industrial-tertiary';
      },
      get valueClass() {
        const color =
          statusMaps.health[status.value.healthStatus]?.color || 'text-industrial-tertiary';
  ...
  if (item.key === 'linkTestResult') {
    return {
      ...item,
      get icon() {
        return statusMaps.linkTest[status.value.linkTestResult]?.icon || 'help';
      },
      get iconColor() {
        return (
          statusMaps.linkTest[status.value.linkTestResult]?.color || 'text-industrial-tertiary'
        );
      },
      get valueClass() {
        const color =
