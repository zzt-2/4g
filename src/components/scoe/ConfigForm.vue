  ...
  <div class="bg-industrial-panel h-full flex flex-col">
    ...
    <div class="relative bg-industrial-secondary">
      <div class="flex flex-shrink-0 no-wrap items-center gap-6 p-4">
        <div class="flex items-center gap-2">
          <!-- SCOE标识 -->
          <div class="flex items-center gap-2">
            <label class="w-32 text-industrial-secondary text-sm whitespace-nowrap">SCOE标识:</label>
            <q-input v-model="scoeStore.globalConfig.scoeIdentifier" class="w-32" dense outlined
              bg-color="industrial-secondary" input-class="text-industrial-primary text-sm" />
          </div>

          <!-- 卫星ID -->
          <div class="flex items-center gap-2">
            <label class="w-32 text-industrial-secondary text-sm whitespace-nowrap">卫星ID:</label>
            <q-input v-model="localSatelliteConfig.satelliteId" class="w-32" dense outlined
              bg-color="industrial-secondary" input-class="text-industrial-primary text-sm"
    ...
    <div class="flex-1 flex flex-col no-wrap min-h-0 overflow-y-auto p-4 gap-4">
      ...
      <template v-else>
        ...
        <!-- 测试工具 -->
        <TestTool class="flex-1 min-h-500px p-3 bg-industrial-secondary" />
      </template>
    </div>
...
import { defaultScoeSatelliteConfig, receiveConfigFields, sendConfigFields } from '../../types/scoe'
...
const sendConfigValidationErrors = computed(() => {
  return sendConfigFields.map(field => field.must && !(localSatelliteConfig.value.sendConfig as ScoeSendConfig)[field.key as keyof ScoeSendConfig])
})

const receiveConfigValidationErrors = computed(() => {
  return receiveConfigFields.map(field => field.must && !(localSatelliteConfig.value.receiveConfig as ScoeReceiveConfig)[field.key as keyof ScoeReceiveConfig])
})

// 监听选中配置变化，更新本地表单数据
watch(() => scoeStore.selectedConfig, (newConfig) => {
  if (newConfig) {
