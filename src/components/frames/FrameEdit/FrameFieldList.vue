    ...
    <div class="flex flex-col flex-nowrap h-full max-h-full w-full overflow-hidden">
        <div class="flex items-center justify-between mb-1 flex-shrink-0">
            ...
            <button
                ...
                @click="dragSort.toggleDragMode" title="切换排序模式">
        ...
        <div class="flex-grow min-h-0 border border-industrial rounded-md bg-industrial-panel overflow-auto">
                ...
                <TransitionGroup name="field-list" tag="div" class="p-1.5">
                    <template v-for="(field, index) in fieldStore.fields" :key="field.id">
                        ...
                        <div v-if="dragSort.shouldShowDropIndicator('before', index)"
                            class="h-1 bg-industrial-accent rounded mb-1 transition-all duration-200"></div>

                        <div class="flex items-center mb-1 bg-industrial-secondary rounded-md border border-industrial h-8 cursor-pointer select-none transition-colors duration-200 hover:border-industrial-accent"
                            :class="{
                            ...
                            <!-- 拖拽手柄（排序模式下显示） -->
                            <div v-if="dragSort.options.value.enabled"
                            ...
                            <div class="flex-grow h-[32px] overflow-hidden flex flex-col justify-center py-0.5 px-1.5"
                                :class="{
                                    'max-w-[450px] w-[calc(100%-50px)]': !dragSort.options.value.enabled,
                                ...
                                <div class="flex items-center overflow-hidden">
                                    ...
                                    <span v-if="field.validOption && field.validOption.isChecksum"
                            ...
                            <div v-if="!dragSort.options.value.enabled" class="flex h-full flex-shrink-0">
                                <button
                                    ...
                                    @click.stop="handleDuplicate(index)" title="复制">
                                ...
                                <button
                                    ...
                                    @click.stop="removeFieldDirect(index)" title="删除">
                    ...
                    <!-- 在列表末尾显示插入指示器 -->
                    <div v-if="dragSort.shouldShowDropIndicator('after', fieldStore.fields.length - 1)"
                        class="h-0.5 bg-industrial-accent rounded transition-all duration-200"></div>
                </TransitionGroup>
            </template>
        ...
        <div class="p-2 border-t border-industrial mt-2 flex-shrink-0">
            <button
                ...
                @click="startEditField(null)" :disabled="dragSort.options.value.enabled"
