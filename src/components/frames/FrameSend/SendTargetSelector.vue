...
<template>
  <div class="send-target-selector">
    <q-select
      ...
      class="w-full"
      :disable="disabled"
      :placeholder="placeholder || '选择发送目标'"
      input-class="py-0.5 px-2 text-xs"
      hide-bottom-space
    >
      <template #prepend>
        <q-icon name="send_to_mobile" size="xs" class="text-industrial-accent" />
      </template>
      ...
      <template #selected-item="scope">
        <div class="flex items-center">
          <q-icon
            ...
            "
            size="xs"
            class="mr-2 text-industrial-accent"
          />
          <span class="text-xs text-industrial-primary">{{ scope.opt.name }}</span>
          <span
