...
<template>
    <div class="send-target-selector">
        <q-select v-model="modelValue" :options="connectionTargetsStore.availableTargets" option-value="id"
            ...
            class="w-full" :disable="disabled" :placeholder="placeholder || '选择发送目标'" input-class="py-0.5 px-2 text-xs"
            hide-bottom-space>
                ...
                <div class="flex items-center">
                    <q-icon :name="scope.opt.type === 'serial'
                        ...
                        : scope.opt.type === 'network'
                            ? 'lan'
                            : 'device_hub'
                        " size="xs" class="mr-2 text-industrial-accent" />
                    <span class="text-xs"
                        :class="scope.opt.status === 'connected' ? 'text-positive' : 'text-negative'">{{ scope.opt.name
                        }}</span>
