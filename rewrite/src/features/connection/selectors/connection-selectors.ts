...
import { getReconnectPolicy, type ReconnectPolicy } from '../core/reconnect';
...
export function selectReconnectStatus(
  snapshot: ReadonlyConnectionStateSnapshot,
  connectionId: string,
  ...
  const isActive = fact.reconnectAttempt !== undefined;

  return {
    isActive,
    attempt: fact.reconnectAttempt ?? 0,
    ...(fact.reconnectNextAt ? { nextAt: fact.reconnectNextAt } : {}),
    policy,
  };
}
