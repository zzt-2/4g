...
// 任务编辑弹窗：从 ExecutionListPage 内联抽出的独立组件（S011）。
...
import type { useTaskEditor } from '@/features/task/composables/use-task-editor';

interface Props {
  /** v-model 控制显隐 */
  modelValue: boolean;
  /** 编辑器实例（page 持有的 use-task-editor 返回值） */
...
<template>
  <q-dialog :model-value="modelValue" @update:model-value="emit('update:modelValue', $event)" @hide="onDialogHide">
    <q-card class="rw-dialog-xl">
      <q-card-section>
        <div class="text-h6">{{ props.editor.editingInstanceId.value ? '编辑任务' : '新建任务' }}</div>
      </q-card-section>

      <q-card-section class="pt-0 rw-dialog-scroll-body">
        <q-form ref="editorFormRef" @submit.prevent>
          <div class="flex flex-col gap-4">
            <q-input :model-value="props.editor.taskName.value" dense outlined label="任务名称"
              :rules="[(val: string) => !!val || '请输入任务名称']"
              @update:model-value="(v: string | number | null) => patchField('taskName', String(v ?? ''))" />

            <div>
              <span class="rw-text-label text-xs">调度类型</span>
              <q-select :model-value="props.editor.scheduleKind.value" :options="SCHEDULE_KIND_OPTIONS" emit-value map-options
            ...
            <template v-if="props.editor.scheduleKind.value === 'timer'">
              <q-input :model-value="props.editor.timerIntervalMs.value" dense outlined type="number" label="间隔 (ms)"
                :rules="[(val: number) => val > 0 || '间隔必须大于0']"
                @update:model-value="(v: string | number | null) => patchField('timerIntervalMs', Number(v) || 0)" />
              <div class="flex items-center gap-3">
                <q-toggle :model-value="props.editor.timerInfinite.value" label="无限循环"
                  @update:model-value="(v: boolean) => patchField('timerInfinite', v)" />
                <q-input v-if="!props.editor.timerInfinite.value" :model-value="props.editor.timerIterations.value" dense
              ...
              <div class="flex flex-col gap-2">
                <div class="flex items-center justify-between">
                  ...
                  <q-btn flat dense no-caps icon="o_add" label="添加条件" size="sm" color="primary"
                    @click="props.editor.addEventCondition()" />
                </div>
                <div v-for="(cond, ci) in props.editor.eventConditions.value" :key="ci" class="flex items-center gap-1">
                  <ConditionTermEditor :model-value="cond" :frame-service="frameService" :show-logic-operator="ci > 0"
                    direction="receive" @update:model-value="props.editor.updateEventCondition(ci, $event)" />
                  <q-btn flat round dense icon="o_close" size="xs" color="negative"
                    @click="props.editor.removeEventCondition(ci)" />
                </div>
                <div v-if="props.editor.eventConditions.value.length === 0" class="rw-text-desc text-xs">至少添加一条触发条件</div>
                <q-input :model-value="props.editor.eventCooldownMs.value" dense outlined type="number" label="冷却时间 (ms)"
            ...
            <div>
              ...
              <SendTargetSelector :model-value="props.editor.defaultTargetId.value" :connection-service="connectionService"
                class="mt-1" @update:model-value="(v: string | null) => patchField('defaultTargetId', v)" />
            ...
            <div class="flex flex-col gap-2">
              ...
              <q-expansion-item v-for="(step, si) in props.editor.steps.value" :key="step.id" dense switch-toggle-side
            ...
            <AdvancedConfigPanel :stop-condition="props.editor.stopCondition.value" :error-policy="props.editor.errorPolicy.value"
            ...
            <template v-if="props.editor.validationIssues.value.length > 0">
              ...
              <div class="flex flex-col gap-1">
                <div v-for="(issue, ii) in props.editor.validationIssues.value" :key="ii" class="text-xs"
      ...
      <q-card-actions align="right">
        ...
        <q-btn unelevated no-caps color="primary" label="保存" :loading="props.editor.isSaving.value" @click="emit('save')" />
        <q-btn unelevated no-caps color="positive" label="保存并启动" :loading="props.editor.isSaving.value"
