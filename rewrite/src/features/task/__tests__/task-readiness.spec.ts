...
describe('validateTaskDefinition', () => {
  ...
  it('duplicate step ids -> error', () => {
    const def = {
      ...timedTaskDef(),
      steps: [
        { id: 'dup-step', kind: 'send' as const, sendConfig: { frameId: 'f1', fieldValues: {} } },
        { id: 'dup-step', kind: 'send' as const, sendConfig: { frameId: 'f2', fieldValues: {} } },
      ],
    };
    const issues = validateTaskDefinition(def);
  ...
  it('send step missing frameId -> error', () => {
    const def = {
      ...timedTaskDef(),
      steps: [
        { id: 's1', kind: 'send' as const, sendConfig: { frameId: '', fieldValues: {} } },
      ],
    };
    const issues = validateTaskDefinition(def);
  ...
  it('wait-condition step missing condition -> error', () => {
    const def = {
      ...timedTaskDef(),
      steps: [
        {
          ...
          waitConfig: { condition: null as any, timeoutMs: 5000, onTimeout: 'fail' as const },
        },
      ],
    };
    const issues = validateTaskDefinition(def);
  ...
  it('delay step invalid duration -> error', () => {
    const def = {
      ...timedTaskDef(),
      steps: [
        { id: 'd1', kind: 'delay' as const, delayConfig: { durationMs: 0 } },
      ],
    };
    const issues = validateTaskDefinition(def);
...
describe('Step builders', () => {
  it('createSendStep creates correct structure', () => {
    const step = createSendStep(
      { frameId: 'frame-A', fieldValues: { v: 42 }, targetId: 'target-B' },
      { id: 'my-send', name: 'My Send' },
    );
    expect(step.kind).toBe('send');
    expect(step.id).toBe('my-send');
    expect(step.name).toBe('My Send');
    expect(step.sendConfig.frameId).toBe('frame-A');
    expect(step.sendConfig.targetId).toBe('target-B');
  });
  ...
  it('createDelayStep creates correct structure', () => {
    ...
    expect(step.delayConfig.durationMs).toBe(2000);
  });
  ...
  it('createWaitConditionStep creates correct structure', () => {
    ...
    const step = createWaitConditionStep(
      { condition: cond, timeoutMs: 3000, onTimeout: 'fail' },
      { id: 'my-wait' },
    );
    expect(step.kind).toBe('wait-condition');
    expect(step.id).toBe('my-wait');
    expect(step.waitConfig.condition).toEqual(cond);
    expect(step.waitConfig.timeoutMs).toBe(3000);
    expect(step.waitConfig.onTimeout).toBe('fail');
  });

  it('createTaskDefinition creates valid definition', () => {
    const step = createSendStep({ frameId: 'f1', fieldValues: {} }, { id: 's1' });
  ...
  it('cloneStepDefinition returns independent copy', () => {
    ...
    const mutable = cloned as { sendConfig: { frameId: string } };
    mutable.sendConfig.frameId = 'MUTATED';
    expect(step.sendConfig.frameId).toBe('f1');
  });
});
